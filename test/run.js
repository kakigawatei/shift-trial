...
head("1. 店舗ごとの設定（お客様の数字どおりか）");
{
  ...
  eq(oc.pos.map((p) => p.label),
     ["オープン", "カレーパン", "昼①", "昼②", "昼③", "昼④", "午後①", "午後②", "午後③", "外販"],
     "落合のポジション10種");
  ...
  eq(oc.band.wd.p1, [300, 570], "平日オープン 5:00-9:30");
...
head("2. 必要人数の読み取り（平日／土日祝の切り替え）");
{
  ...
  eq(S.posFor(17).length, 10, "平日は10ポジション");
  eq(S.posFor(23).map((p) => p.label), ["オープン", "カレーパン", "昼①", "昼②", "昼③", "昼④", "午後①", "午後②", "午後③"], "土日祝は外販を除く9ポジション");
  ...
  eq(S.posFor(17).length, 10, "おはし二日町は曜日で減るポジションなし");
}
...
(async () => {
  ...
  eq(S.POS[0].label, "オープン", "ポジションが落合のものに入れ替わる");
...
async function part5() {
  ...
  eq(early && early.pos, "p1", "5時スタートの人は『オープン』に置かれる");
